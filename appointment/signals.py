...
def appointment_status_notification_signal(
    ...
    if created:
        ...
        logger.info(
            f"Signal: Appointment #{instance.id} created. "
            f"Scheduling Telegram notification task on database commit."
        )
        ...
        if instance.status != original_status:
            ...
            logger.info(
                f"Signal: Appointment #{instance.id} status changed "
                f"({original_status} -> {instance.status}). "
                f"Scheduling update notification task on database commit."
            )
